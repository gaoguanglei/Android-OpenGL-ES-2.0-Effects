...
void opengl_init(int w, int h)
{
	...
	if(statue_mesh.program == NULL)
	{
		...
		statue_mesh.texs_idxs[0] = create_texture("media/AmbientOcclusion_1024.dds");
		statue_mesh.texs_idxs[1] = create_cube_texture("media/grace_diffuse_cube.dds");
	...
	if(background_mesh.program == NULL)
	{
		...
		background_mesh.texs_idxs[0] = create_cube_texture("media/grace_cube.dds");
...
void opengl_display()
{
	...
	view.look_at(vec3(0,0,distance_z), vec3(0,0,0), vec3(0,1,0));
