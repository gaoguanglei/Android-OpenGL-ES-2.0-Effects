...
struct vec4 : public vec3
{
	...
	vec4(const vec3& v, float d) : vec3(v), w(d){}
};
...
vec4	light_pos = vec4(100,0,0,1);
vec4	light_dir;
vec4	view_pos;

GLuint texture_obj,texture_bump;

float	distance_z = 20.3f;
...
void opengl_init(int w, int h)
{
	...
	GLuint num_indx = shape::create_sphere(14, 10.f, &pos_data, &normal_data, &tex_data, &inds, num_ves);

	texture_obj = create_texture("media/Fieldstone.tga");
	texture_bump = create_texture("media/FieldstoneBumpDOT3.tga");

	vec3* targents = new vec3[num_ves];
	vec3* binormal = new vec3[num_ves];

	computer_targent_space((vec3*)normal_data, 
		(vec3*)pos_data, 
		targents, 
		binormal, 
		tex_data, 
	...
	glBufferData(GL_ARRAY_BUFFER, 4*2*num_ves, tex_data, GL_STATIC_DRAW);

	glBindBuffer(GL_ARRAY_BUFFER, object_mesh.attribute_vbos[POSITION]);
	glBufferData(GL_ARRAY_BUFFER, 4*3*num_ves, pos_data, GL_STATIC_DRAW);

	glBindBuffer(GL_ARRAY_BUFFER, object_mesh.attribute_vbos[NORMAL]);
	glBufferData(GL_ARRAY_BUFFER, 4*3*num_ves, normal_data, GL_STATIC_DRAW);

	glBindBuffer(GL_ARRAY_BUFFER, object_mesh.attribute_vbos[TARGENT]);
	glBufferData(GL_ARRAY_BUFFER, 4*3*num_ves, targents, GL_STATIC_DRAW);

	glBindBuffer(GL_ARRAY_BUFFER, object_mesh.attribute_vbos[BINORMAL]);
	glBufferData(GL_ARRAY_BUFFER, 4*3*num_ves,binormal, GL_STATIC_DRAW);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, object_mesh.attribute_vbos[INDEX]);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 2*num_indx, inds, GL_STATIC_DRAW);

	object_mesh.num_faces = num_indx/3;
...
void opengl_display()
{
	...
	view_pos = vec4(viewpos, 1.0);

	vec3 lightpos = world_view_matrix.inverse() * vec3(0,0,0);
	light_pos = vec4(lightpos, 1.0f);
